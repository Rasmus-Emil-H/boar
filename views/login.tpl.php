<div>
    ...
    <form class="login-form center flex-column" method="POST" action="/auth/login">
        <h1 class="header mb-4"><?= \app\core\src\miscellaneous\CoreFunctions::hs(\app\core\src\miscellaneous\CoreFunctions::app()->getConfig()->get('appName')); ?></h1>
        ...
        <?= (new \app\core\src\tokens\CsrfToken())->insertHiddenToken(); ?>
            <button type="submit" class="btn btn-primary btn-lg mt-2 w-100"><?= \app\core\src\miscellaneous\CoreFunctions::ths('Log in'); ?></button>
        <a href="signup" class="btn btn-primary btn-lg mt-2 w-100"><?= \app\core\src\miscellaneous\CoreFunctions::ths('Create account'); ?></a>
    </form>
</div>